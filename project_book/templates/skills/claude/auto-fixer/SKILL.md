---
name: auto-fixer
description: "Use when the Architect asks Claude Code to work autonomously. Runs sequential Fixer -> Netrunner -> Fixer-review cycles until the goal is reached or a true blocker appears."
---
...
6. Launch one autonomous Netrunner via the Claude Code `Agent` tool
   (`subagent_type: "general-purpose"`) for the prepared session. Give the
   subagent the exact session path and tell it to activate `/init-netrunner`.
